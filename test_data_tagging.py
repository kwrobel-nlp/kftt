...
def segments_to_tokens(segments):
    ...
    for char, pred, space_before in segments:
        ...
        if pred >= 1:
            tokens.append((token, pred))
            token = ''
    assert token == ''
    return tokens

def merge(segments, data, writer):
    ktext = KText.load(data)
    merge2(segments, ktext, writer)
    
def merge2(segments, ktext: KText, writer=None) -> KText:
    ...
    predicted_tokens = []
    start_offset = 0
    for token, token_pred in segmentation_tokens:
        ...
        plain_token.sentence_end = token_pred == 2
    ...
    for token in ktext.tokens:
        ...
        if writer: writer.write("\t".join(
            [token.form, space, '_'.join(sorted(tags)), '_'.join(sorted(poss)), str(ktext.year)]))
        # TODO: save sentence_end or JSONL with everything
